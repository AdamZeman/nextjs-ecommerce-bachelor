...
function EditSpecial({ special, onClose, setSpecials}:PropsType) {
    ...
    const handleEditProduct = async () => {
        ...
        const res = await fetch(`/api/iom/special/EditSpecial`, {
            method: 'POST',
            headers: {
    ...
    const handleDelete = async () => {
        const res = await fetch(`/api/iom/special/DeleteSpecial`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (res.ok){
            setSpecials((prevItems) => prevItems.filter((item) => item.id !== special.id));
            onClose()
        }
    };
    ...
    return (
        <div className="bg-white p-8 shadow-md max-w-2xl w-[600px] rounded-xl">
            <div className="flex justify-between mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Edit Special</h2>
                <button
                    onClick={onClose}
